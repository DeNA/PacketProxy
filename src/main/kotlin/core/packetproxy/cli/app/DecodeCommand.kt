...
/** stdin を受け取り、指定エンコーダで decode して stdout へ出力するワンショットコマンド。 */
@Command(
  name = "decode",
  mixinStandardHelpOptions = true,
  description = ["Decode stdin using the specified encoder and write result to stdout"],
)
class DecodeCommand : Callable<Int> {
  ...
  @Option(
    names = ["--response"],
    description = ["Use server-response direction (decodeServerResponse). Default: client-request."],
  )
  ...
  override fun call(): Int =
    Codec.run(
      ...
      direction = if (response) Codec.Direction.DECODE_RESPONSE else Codec.Direction.DECODE_REQUEST,
      text = text,
      inFile = inFile,
      outFile = outFile,
    )
