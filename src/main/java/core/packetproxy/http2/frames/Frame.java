...
public class Frame {
	...
	protected static Type TYPE = Type.Unassigned;
	protected static byte FLAG_EXTRA = 0x40; /* internal use only */
	...
	public Frame(byte[] data) throws Exception {
		...
		streamId = (int) ((buffer[0] & 0x7f) << 24 | (buffer[1] & 0xff) << 16 | (buffer[2] & 0xff) << 8
				| (buffer[3] & 0xff));
	...
	public byte[] getOrigPayload() throws Exception {
		return this.origPayload;
	}

	public byte[] getExtra() throws Exception {
		return this.extra;
	}
