...
public abstract class PnSpace {
	...
	public synchronized void addSendFrameFirst(Frame frame) {
		QuicPacketBuilder builder = QuicPacketBuilder.getBuilder().setPnSpaceType(this.pnSpaceType)
	...
	public synchronized LostPackets detectAndRemoveLostPackets() {
		...
		for (SentPacket unAcked : this.sentPackets.getUnAckedPackets()) {
			...
			/*
			 * Mark packet as lost, or set time when it should be marked. Note: The use of
			 * kPacketThreshold here assumes that there were no sender-induced gaps in the
			 * packet number space.
			 */
