...
public class ServerHandshake implements Handshake {
    ...
    static private final TlsSessionRegistry tlsSessionRegistry = new TlsSessionRegistryImpl();
    ...
    public void startHandshake(String sniName) throws Exception {
        ...
        this.engine = new TlsServerEngine(certs, key, new MyServerMessageSender(), new MyTlsStatusEventHandler(), tlsSessionRegistry);
    ...
    public void received(Message message) throws Exception {
        if (message instanceof ClientHello) {
            ...
            ch.getExtensions().stream()
                    .filter(ext -> ext instanceof ServerNameExtension)
                    .findFirst()
    ...
    class MyServerMessageSender implements ServerMessageSender {
        ...
        public void send(ServerHello serverHello) throws IOException {
            MessagesToCryptoFrames stream = conn.getPnSpace(Constants.PnSpaceType.PnSpaceInitial).getMsgToFrameCryptoStream();
        ...
        public void send(EncryptedExtensions encryptedExtensions) throws IOException {
            MessagesToCryptoFrames stream = conn.getPnSpace(Constants.PnSpaceType.PnSpaceHandshake).getMsgToFrameCryptoStream();
        ...
        public void send(NewSessionTicketMessage ticket) throws IOException {
            ...
            MessagesToCryptoFrames stream = conn.getPnSpace(Constants.PnSpaceType.PnSpaceApplicationData).getMsgToFrameCryptoStream();
            stream.write(ticket);
            Frames frames = stream.toCryptoFrames();
