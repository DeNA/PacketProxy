...
public class Frames implements Iterable<Frame> {
    ...
    static public final Frames empty = new Frames(ImmutableList.of());

    static public Frames parse(byte[] bytes) {
        return Frames.parse(ByteBuffer.wrap(bytes));
    }

    static public Frames parse(ByteBuffer buffer) {
        try {
    ...
    static public Frames of(Frame e1) {
        return new Frames(ImmutableList.of(e1));
    }

    static public Frames of(Frame e1, Frame e2) {
        return new Frames(ImmutableList.of(e1, e2));
    }

    static public Frames of(Frame e1, Frame e2, Frame e3) {
        return new Frames(ImmutableList.of(e1, e2, e3));
    }

    static public Frames of(List<Frame> frameList) {
    ...
    public Optional<AckFrame> getAckFrame() {
        return frames.stream()
                .filter(f -> f instanceof AckFrame)
                .map(f -> (AckFrame)f)
                .findFirst();
    }
