...
public class Keys {
    ...
    static private Path logDir  = Paths.get(System.getProperty("user.home") + "/.packetproxy/logs");
    static private Path keylogFile = Paths.get(logDir + "/quic_tls.keylog");

    /**
     * Wiresharkのメニューから、Preference > Advanced > tls.keylog_file に keylog ファイルのパスを入力した後、
     * View > Reload で再読み込みするとQUICパケットのペイロードが読めるようになる
     */
        ...
        if (this.clientRandom != null && this.clientKeys.hasHandshakeKey() && this.clientKeys.hasApplicationKey()) {
            try {
                ...
                try (FileWriter file = new FileWriter(keylogFile.toFile())) {
                    ...
                    file.write(String.format("CLIENT_HANDSHAKE_TRAFFIC_SECRET %s %s\n", clientRandomStr, Hex.encodeHexString(this.clientKeys.getHandshakeKey().getSecret())));
                    file.write(String.format("SERVER_HANDSHAKE_TRAFFIC_SECRET %s %s\n", clientRandomStr, Hex.encodeHexString(this.serverKeys.getHandshakeKey().getSecret())));
                    file.write(String.format("CLIENT_TRAFFIC_SECRET_0 %s %s\n", clientRandomStr, Hex.encodeHexString(this.clientKeys.getApplicationKey().getSecret())));
                    file.write(String.format("SERVER_TRAFFIC_SECRET_0 %s %s\n", clientRandomStr, Hex.encodeHexString(this.serverKeys.getApplicationKey().getSecret())));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
