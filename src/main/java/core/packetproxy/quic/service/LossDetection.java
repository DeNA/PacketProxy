...
public class LossDetection {
	...
	public synchronized void setLossDetectionTimer() {
		...
		if (earliestLossTime != Instant.MAX) {
			/*
			 * Time threshold loss detection. パケットロスが発生する時間にタイマーをセット
			 */
	...
	private synchronized void onLossDetectionTimeout() {
		...
		if (earliestLossTime != Instant.MAX) {
			/*
			 * Time threshold loss Detection パケットロスが発生したので該当パケットを再送
			 */
		...
		if (!this.conn.getPnSpaces().hasAnyAckElicitingPacket()) {
			if (this.conn.peerAwaitingAddressValidation()) {
				 ...
				 * anti-amplification credit, a Handshake packet proves address ownership.
			...
			/*
			 * PTO. Send new data if available, else retransmit old data. If neither is
			 * available, send a single PING frame. AckElicitingパケットを投げたのに、Ackが返ってこないので催促する
			 */
