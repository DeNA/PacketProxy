...
public class TruncatedPacketNumber {
    ...
    static public byte[] unmaskTruncatedPacketNumber(byte[] truncatedPacketNumber, byte[] maskKey) {
        return maskTruncatedPacketNumber(truncatedPacketNumber, maskKey);
    }

    static public byte[] maskTruncatedPacketNumber(byte[] truncatedPacketNumber, byte[] maskKey) {
    ...
    static private byte[] encode(PacketNumber packetNumber, PacketNumber largestAckPn) {
        /*
    ...
    static private long decode(byte[] truncatedPacketNumberBytes, PacketNumber largestAckPn) {
    ...
    static private byte[] truncate(long packetNumber, int byteLength) {
        if (byteLength == 0 || byteLength == 1) {
            ...
            return new byte[] { (byte)((packetNumber >> 16) & 0xff), (byte)((packetNumber >> 8) & 0xff), (byte)(packetNumber & 0xff) };
        }
        else if (byteLength >= 4) {
            return new byte[] { (byte)((packetNumber >> 24) & 0xff), (byte)((packetNumber >> 16) & 0xff), (byte)((packetNumber >> 8) & 0xff), (byte)(packetNumber & 0xff) };
        }
        System.err.println(String.format("[Error] can't encode packetNumber from Long to ByteArray (byteLength=%d)", byteLength));
        return null;
    }
}
