...
public class LongHeaderPnSpacePacket extends LongHeaderPacket implements PnSpacePacket {
    ...
    protected LongHeaderPnSpacePacket(byte type, int version, ConnectionIdPair connIdPair, PacketNumber packetNumber, byte[] payload) {
    ...
    public byte[] getBytes(Key key, PacketNumber largestAckedPn) {
        ...
        byte[] payloadLength = VariableLengthInteger.of(truncatedPn.length + payload.length + 16 /* GCM auth hash */).getBytes();
    ...
    public String toString() {
        return String.format("LongHeaderPacket(version=%d, connIdPair=%s, packetNumber=%s, payload=%s",
                this.version,
                this.connectionIdPair,
                this.packetNumber,
