...
public class Binary {
	 ...
	 * PacketProxyUtility.getInstance(); try { util.packetProxyLog(new Binary(new
	 * HexString("616263")).toHexString(2).toString()); util.packetProxyLog(new
	 * Binary(new HexString("61 62 63")).toHexString(2).toString());
	 * util.packetProxyLog(new Binary(new
	 * HexString("61  62    63")).toHexString(2).toString());
	 * util.packetProxyLog(new Binary(new
	 * byte[]{0x61,0x62,0x63}).toHexString(2).toString()); util.packetProxyLog(new
	 * Binary(new HexString("616263a283")).toAsciiString(2).toString());
	 * util.packetProxyLog(new Binary(new
	 * HexString("61 62 63 a2 83")).toAsciiString(2).toString());
	 * util.packetProxyLog(new Binary(new
	 * HexString("61  62    63   a283")).toAsciiString(2).toString());
	 * util.packetProxyLog(new Binary(new
	 * byte[]{0x61,0x62,0x63,(byte)0xA2,(byte)0x83}).toAsciiString(2).toString());
	 * util.packetProxyLog(new
	 * Binary("hoge\nhogeあああ".getBytes()).toHexString().toString());
	 *
	 * String data = new String("\n"); for (int i = 0; i < 20; i++) { data = data +
	 * data; } long start = System.currentTimeMillis(); new
	 * Binary(data.getBytes()).toHexString(); long end = System.currentTimeMillis();
	 * util.packetProxyLog("size: " + data.length() + ", time: " + ((end - start) /
	 * 1000.0)); } catch (Exception e) { e.printStackTrace(); } }
	 */
