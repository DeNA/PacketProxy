...
/**
 * ボイヤー・ムーアアルゴリズムの簡易版 検索するパターンの長さをm, テキストの長さを n として、時間計算量の平均は O(n) となる。 Good
 * Suffix Ruleを実装していないので最悪のケースでは O(nm) となる。(Good Suffix Ruleを実装している場合は O(n + m)
 * ) ただし、よほど変なテキストを食わせない限りは最悪のケースには陥らず、概ね n に比例する時間で計算が完了する。
 *
 * 4KBのランダムな文字列から12バイト程度の部分文字列を検索する場合、Bruteforceで検索するより平均して2倍速かった。 (1500ns ->
 * 800ns) //TODO Good Suffix Ruleを実装する場合は、JMHなどでベンチマークをとって実際に速くなることを確認する。
 *
 * @author 星本
 * @version 0.1.0
 */
public class BoyerMoore {
	...
	/**
	 * 与えられたバイト列の中から、インスタンス化時で指定したパターン (バイト列) を検索する はじめに見つかった部分文字列の開始位置のインデックスを返す。
	 *
	 * @param text
	 *            検索対象のテキスト
	...
	public int searchIn(byte[] text, int offset, int endpos) {
		...
		if (pattern.length > 0 && endpos >= offset && pattern != null && pattern.length <= text.length) {
