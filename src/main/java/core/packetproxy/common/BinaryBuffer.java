	...
	*/
	...
	public void remove(int index, int length) {
		...
		if (data_size < index + length) {
			PacketProxyUtility.getInstance().packetProxyLog(String.format("[Error] Something wrong (%d < %d + %d)", data_size, index , length));
			return;
		}
