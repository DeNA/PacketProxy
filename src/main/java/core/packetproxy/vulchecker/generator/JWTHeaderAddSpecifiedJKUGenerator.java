...
public class JWTHeaderAddSpecifiedJKUGenerator extends Generator {
    ...
    public String getName() {
        return "Header: jku: ユーザが指定したjkuを注入";
    }
    ...
    public String generate(String inputData) throws Exception {
        ...
        int width = 400;
        int height = 300;
        dlg.setBounds(rect.x + rect.width/2 - width/2, rect.y + rect.height/2 - height/2, width, height); /* ド真ん中 */

        JPanel labels = new JPanel();
        labels.setLayout(new BoxLayout(labels, BoxLayout.X_AXIS));
        labels.add(new JLabel("JKUを指定してください"));
        ...
        jwt.setHeaderValue("jku", area.getText());
