...
public class Http3 {
    ...
    public byte[] generateReqHttp(HttpRaw httpRaw) throws Exception {
        ...
        this.clientDecoder.decode(httpRaw.getStreamId().getId(), httpRaw.getEncodedHeader()).forEach(rethrow(metaData -> {
    ...
    public byte[] generateResHttp(HttpRaw httpRaw) throws Exception {
        ByteArrayOutputStream http = new ByteArrayOutputStream();
        this.serverDecoder.decode(httpRaw.getStreamId().getId(), httpRaw.getEncodedHeader()).forEach(rethrow(metaData -> {
            MetaData.Response res = (MetaData.Response) metaData;
            http.write(String.format("HTTP/3 %d %s\r\n", res.getStatus(), HttpStatus.getMessage(res.getStatus())).getBytes());
