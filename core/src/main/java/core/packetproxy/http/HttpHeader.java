...
public class HttpHeader {
	...
	public static int calcHeaderSize(byte[] data) {
		for (int i = 0; i < data.length; i++) {

			if (i <= data.length - 4 && data[i] == '\r' && data[i + 1] == '\n' && data[i + 2] == '\r'
					&& data[i + 3] == '\n') {
	...
	public List<HeaderField> getAll(String name) {
		return fields.stream().filter(h -> h.getName().equalsIgnoreCase(name)).collect(Collectors.toList());
	}
	...
	public byte[] toByteArray() {
		return fields.stream().map(h -> h.toString()).collect(Collectors.joining(newLineSymbol, "", newLineSymbol))
				.getBytes();
	}
