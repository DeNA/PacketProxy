...
public class Pto {
	...
	public ImmutablePair<Instant, PnSpaceType> getPtoTimeAndSpace() {
		...
		long duration = (smoothedRtt + Math.max(4 * rttVar, Constants.kGranularity))
				* (long) (Math.pow(2, this.ptoCount));
		...
		if (this.conn.peerAwaitingAddressValidation()) {
			...
			if (this.conn.getHandshakeState().hasNoHandshakeKeys()) {
				...
				return ImmutablePair.of(Instant.now().plusMillis(duration), PnSpaceInitial);
			} else {

				return ImmutablePair.of(Instant.now().plusMillis(duration), PnSpaceHandshake);
			}
		}
